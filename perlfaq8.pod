...
managed to finish its output without filling up the disk:
    ...
    END {
	close STDOUT   or   die "stdout close failed: $!";
    }
...
You're confusing the purpose of C<system()> and backticks (`` or qx()). C<system()>
runs a command and returns exit status information (as a 16 bit value:
the low 7 bits are the signal the process died from, if any, and
the high 8 bits are the actual exit value). Backticks command qx() run a
...
This doesn't work:
    ...
    open STDERR, ">&STDOUT";
    $alloutput = qx(cmd args);  # stderr still escapes
...
To capture a command's STDOUT but discard its STDERR:
    ...
    $output = qx(cmd 2>/dev/null);              # either with backticks
    $pid = open PH, "cmd 2>/dev/null |";        # or with an open pipe
    while ( <PH> ) { }                          #    plus a read

To capture a command's STDERR but discard its STDOUT:

    $output = qx(cmd 2>&1 1>/dev/null);         # either with backticks
...
but leave its STDOUT to come out our old STDERR:
    ...
    $output = qx(cmd 3>&1 1>&2 2>&3 3>&-);      # either with backticks
...
like this:
    ...
    my @ok = qx(grep @opts '$search_string' @filenames);
