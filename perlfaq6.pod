...
You can do the same thing with for loop and a match using the
...
being in memory at the end.
    ...
    while ( sysread FH, $_, 8192, length ) {
	for my $record ( m/\G((?s).*?)your_pattern/gc ) {
		# do stuff here.
	}
...
the regular expression for every iteration of the C<for>
...
which works in 5.004 or later.
	...
	PARSER: {
	    m/ \G( \d+\b    )/gcx   and   do { print "number: $1\n";  redo; };
	    m/ \G( \w+      )/gcx   and   do { print "word:   $1\n";  redo; };
	    m/ \G( \s+      )/gcx   and   do { print "space:  $1\n";  redo; };
	    m/ \G( [^\w\d]+ )/gcx   and   do { print "other:  $1\n";  redo; };
	}
    }
...
Or like this:
    ...
    # The above is conceptually similar to:     @chars = $text =~ m/(.)/g;
