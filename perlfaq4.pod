...
easily done by making a C-shell alias, like so:
...
Stringification also destroys arrays.
    ...
    @lines = qx(ls);    # Run ls(1)
    print "@lines";     # PROBLEM - extra blanks
    print @lines;       # right
...
element is unique in a given array:
    ...
    for $element ( @array1, @array2 ) { $count{$element}++ }

    for $element ( keys %count ) {
	push @union, $element;
...
Here's a more extensive illustration using vec():
	...
	my @bytes = unpack "A8" x length($vector), $bits;
...
those using C<grep>:
    ...
    for my $key ( grep /^text:/, keys %hash ) {
	 my $value = $hash{$key}
	...
    }
